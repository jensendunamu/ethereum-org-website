...
Static checks pass on pages that are broken in the browser — a wrong image path, a failed MDX compile, a section that silently didn't render. So run the page and look at it.

1. **Run it.** Start `pnpm dev`, wait for "Ready", load the page. Routes are locale-stripped for the default `en` (`/<route>/`). Confirm **HTTP 200** — a 500 is almost always an MDX or asset error; read the dev log (a non-co-located inline image is the classic one).
2. **Compare to the design.** Screenshot the full page with `playwright-cli`, set it beside the design frames, and hunt for **obvious** gaps — a missing section, a component rendered as raw text, a broken or oversized image, content that didn't appear. Fix and reload until the page faithfully reflects the design.
   - **Hero check.** Design has a side-image hero but the page shows a title-only one? On a markdown page that's a wrong-`template:` bug (you need a Topic template, not `static`); on an App Router page, pass `heroImg` to `PageHero`. Never rebuild a hero with custom markup.
